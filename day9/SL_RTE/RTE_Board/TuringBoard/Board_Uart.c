...
void Board_Uart_RecCallback(Board_Uart_Name_e usart_name)
{
	 ...
	 */
	uint32_t alignedAddr = (uint32_t)UartHandle[usart_name].UsartData.pu8Databuf & ~0x1F;
	SCB_InvalidateDCache_by_Addr((uint32_t*)alignedAddr, UartHandle[usart_name].UsartData.u16Datalength + ((uint32_t)UartHandle[usart_name].UsartData.pu8Databuf - alignedAddr));
