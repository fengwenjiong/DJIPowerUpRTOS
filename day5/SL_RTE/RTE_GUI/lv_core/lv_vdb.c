...
static uint8_t vdb_buf[LV_VDB_SIZE_IN_BYTES] = {0};
...
#if LV_VDB_DOUBLE == 0
	if(vdb.buf == 0)
		vdb.buf = (lv_color_t *)RTE_MEM_Alloc0(MEM_FB,LV_VDB_SIZE_IN_BYTES);
	RTE_AssertParam(vdb.buf);
#else
	vdb[0].buf = (lv_color_t *)RTE_BGetz(MEM_FB,LV_VDB_SIZE_IN_BYTES);
	vdb[1].buf = (lv_color_t *)RTE_BGetz(MEM_FB,LV_VDB_SIZE_IN_BYTES);
