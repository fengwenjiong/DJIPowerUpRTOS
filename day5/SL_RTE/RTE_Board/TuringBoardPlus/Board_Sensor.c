...
int sensor_set_framesize(framesize_t framesize)
{
    ...
    MV_FB_Get()->w = resolution[framesize][0];
    MV_FB_Get()->h = resolution[framesize][1];

    // Set MAIN FB backup width and height.
    MV_FB_Get()->u = resolution[framesize][0];
    MV_FB_Get()->v = resolution[framesize][1];
    return 0;
}
...
int sensor_set_windowing(int x, int y, int w, int h)
{
    MV_FB_Get()->x = x;
    MV_FB_Get()->y = y;
    MV_FB_Get()->w = MV_FB_Get()->u = w;
    MV_FB_Get()->h = MV_FB_Get()->v = h;
    return 0;
}
...
static void sensor_check_buffsize()
{
    ...
    if ((MV_FB_Get()->w * MV_FB_Get()->h * bpp) > RTE_MEM_GetDataSize(MV_FB_Get()->pixels)) {
...
int sensor_snapshot(sensor_t *sensor)
{
    ...
    MV_FB_Get()->w = MV_FB_Get()->u;
    MV_FB_Get()->h = MV_FB_Get()->v;
    ...
    switch (sensor->pixformat) {
        ...
        case PIXFORMAT_YUV422:
            ...
            length = (w * h * 2)/4;
            addr = (uint32_t) (MV_FB_Get()->pixels);
            break;
        case PIXFORMAT_BAYER:
...
void sensor_capture_complete(sensor_t *sensor, image_t *image)
{
    ...
    switch (sensor->pixformat) {
        ...
        case PIXFORMAT_JPEG:
            ...
            MV_FB_Get()->bpp = (MAX_XFER_SIZE - __HAL_DMA_GET_COUNTER(&BoardDcmiDma))*4;
            break;
        default:
            break;
    }
		SCB_CleanInvalidateDCache_by_Addr((uint32_t *)MV_FB_Get()->pixels, (MV_FB_Get()->w*MV_FB_Get()->h*MV_FB_Get()->bpp));
		if(sensor->pixformat == PIXFORMAT_GRAYSCALE)//YUV422提取灰度Y
		{
				uint8_t *dst = MV_FB_Get()->pixels;
				uint8_t *src = MV_FB_Get()->pixels;
				uint32_t length = MV_FB_Get()->w * MV_FB_Get()->h;
    ...
    if (image != NULL) {
        image->w = MV_FB_Get()->w;
        image->h = MV_FB_Get()->h;
        image->bpp = MV_FB_Get()->bpp;
        image->pixels = MV_FB_Get()->pixels;
    }
}
