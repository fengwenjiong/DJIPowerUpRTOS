...
int imlib_load_cascade_from_file(cascade_t *cascade, const char *path)
{
    ...
    File_Module_Open(&fp, path);
    File_Module_RWBufOn(&fp);

    /* read detection window size */
    File_Module_ReadData(&fp, &cascade->window, sizeof(cascade->window));

    /* read num stages */
    File_Module_ReadData(&fp, &cascade->n_stages, sizeof(cascade->n_stages));

    cascade->stages_array = RTE_MEM_Alloc0(MEM_DTCM, sizeof(*cascade->stages_array) * cascade->n_stages);
    cascade->stages_thresh_array = RTE_MEM_Alloc0(MEM_DTCM, sizeof(*cascade->stages_thresh_array) * cascade->n_stages);
    ...
    File_Module_ReadData(&fp, cascade->stages_array, sizeof(uint8_t) * cascade->n_stages);
    ...
    cascade->tree_thresh_array = RTE_MEM_Alloc0(MEM_DTCM, sizeof(*cascade->tree_thresh_array) * cascade->n_features);
    cascade->alpha1_array = RTE_MEM_Alloc0(MEM_DTCM, sizeof(*cascade->alpha1_array) * cascade->n_features);
    cascade->alpha2_array = RTE_MEM_Alloc0(MEM_DTCM, sizeof(*cascade->alpha2_array) * cascade->n_features);
    cascade->num_rectangles_array = RTE_MEM_Alloc0(MEM_DTCM, sizeof(*cascade->num_rectangles_array) * cascade->n_features);
    ...
    File_Module_ReadData(&fp, cascade->stages_thresh_array, sizeof(int16_t)*cascade->n_stages);

    /* read features thresholds */
    File_Module_ReadData(&fp, cascade->tree_thresh_array, sizeof(*cascade->tree_thresh_array)*cascade->n_features);

    /* read alpha 1 */
    File_Module_ReadData(&fp, cascade->alpha1_array, sizeof(*cascade->alpha1_array)*cascade->n_features);

    /* read alpha 2 */
    File_Module_ReadData(&fp, cascade->alpha2_array, sizeof(*cascade->alpha2_array)*cascade->n_features);

    /* read num rectangles per feature*/
    File_Module_ReadData(&fp, cascade->num_rectangles_array, sizeof(*cascade->num_rectangles_array)*cascade->n_features);
    ...
    cascade->weights_array = RTE_MEM_Alloc0(MEM_DTCM, sizeof(*cascade->weights_array) * cascade->n_rectangles);
    cascade->rectangles_array = RTE_MEM_Alloc0(MEM_DTCM, sizeof(*cascade->rectangles_array) * cascade->n_rectangles * 4);
    ...
    File_Module_ReadData(&fp, cascade->weights_array, sizeof(*cascade->weights_array)*cascade->n_rectangles);

    /* read rectangles num rectangles * 4 points */
    File_Module_ReadData(&fp, cascade->rectangles_array, sizeof(*cascade->rectangles_array)*cascade->n_rectangles *4);
...
{
    // built-in cascade
    if (strcmp(path, (const char *)"frontalface") == 0) {
