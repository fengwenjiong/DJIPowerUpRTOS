...
void imlib_draw_circle(image_t *img, int cx, int cy, int r, int c, int thickness, bool fill)
{
    if (fill) {
        ...
        int xo = r + thickness0;
        int xi = MATH_MAX(r - thickness1, 0);
        int xi_tmp = xi;
        int y = 0;
        int erro = 1 - xo;
        int erri = 1 - xi;
...
static void scratch_draw_rotated_ellipse(image_t *img, int x, int y, int x_axis, int y_axis, int rotation, bool filled, int c, int thickness)
{
    if ((x_axis > 0) && (y_axis > 0)) {
        if ((x_axis == y_axis) || ((rotation % 180) == 0)) {
            ...
            float theta = fast_atanf(MATH_DIV(y_axis, x_axis) * (-tanf(MATH_DEG2RAD(rotation))));
            float shear_dx = (x_axis * cosf(theta) * cosf(MATH_DEG2RAD(rotation))) - (y_axis * sinf(theta) * sinf(MATH_DEG2RAD(rotation)));
            float shear_dy = (x_axis * cosf(theta) * sinf(MATH_DEG2RAD(rotation))) + (y_axis * sinf(theta) * cosf(MATH_DEG2RAD(rotation)));
            float shear_x_axis = fast_fabsf(shear_dx);
            float shear_y_axis = MATH_DIV((y_axis * x_axis), shear_x_axis);
...
{
    float over_xscale = MATH_DIV(1.0, x_scale), over_yscale = MATH_DIV(1.0f, y_scale), beta = 1 - alpha;
