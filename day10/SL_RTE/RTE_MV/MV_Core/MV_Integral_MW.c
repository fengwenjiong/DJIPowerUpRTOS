...
void imlib_integral_mw_alloc(mw_image_t *sum, int w, int h)
{
    ...
    sum->x_ratio = (1<<16)+1;
    sum->y_ratio = (1<<16)+1;
    sum->data = RTE_MEM_Alloc0(MEM_DTCM, h * sizeof(*sum->data));
    // swap is used when shifting the image pointers
    // to avoid overwriting the image rows in sum->data
    sum->swap = RTE_MEM_Alloc0(MEM_DTCM, h * sizeof(*sum->data));

    for (int i=0; i<h; i++) {
        sum->data[i] = RTE_MEM_Alloc0(MEM_DTCM, w * sizeof(**sum->data));
    }
}
void imlib_integral_mw_free(mw_image_t *sum)
{
    for (int i=0; i<sum->h; i++) {
        RTE_MEM_Free(MEM_DTCM,sum->data[i]);  // Free h lines
    }
    RTE_MEM_Free(MEM_DTCM,sum->data);  // Free data
    RTE_MEM_Free(MEM_DTCM,sum->swap);  // Free swap
}
