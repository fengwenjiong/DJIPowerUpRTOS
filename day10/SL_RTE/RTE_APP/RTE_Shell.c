...
#endif
	RTE_Printf("--------------------------------------------------\r\n");
	RTE_Printf("%10s    Using SHELL Module Nums:%d Allow Max:%d\r\n",
		DEBUG_STR,
		ShellHandle.ShellModuleCnt,
		SHELL_MAX_MODULE);
...
void RTE_Shell_Init(void)
{
	...
	RTE_MEM_Alloc0(MEM_RTE,sizeof(RTE_Shell_Module_t)*SHELL_MAX_MODULE);
	...
	RTE_Shell_AddCommand("system","demon",RTE_Shell_CMD_RTEInfor,"Information about running SL_RTE Example:system.demon");
	RTE_Shell_AddCommand("system","print",RTE_Shell_CMD_Print,"Simple printf Example:system.print(str)");
	RTE_MessageQuene_Init(&ShellQuene,SHELL_BUFSIZE*2);
}
/*************************************************
*** Args:   NULL
*** Function: shell轮询
...
void RTE_Shell_Poll(void *Params)
{
	...
	if(RTE_MessageQuene_Out(&ShellQuene,(uint8_t *)ShellBuffer,NULL) == MSG_NO_ERR)
	{
        ...
        {
            RTE_LOG_WARN(DEBUG_STR,"Input command's args' num error!\r\n");
        }
	}
}
