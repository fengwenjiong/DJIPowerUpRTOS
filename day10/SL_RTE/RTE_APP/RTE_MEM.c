...
static const osMutexAttr_t MutexIDStdioAttr[MEM_N] = {
	{
		"MEMRTEMutex",     // human readable mutex name
		osMutexPrioInherit|osMutexRecursive  ,    					// attr_bits
		NULL,                // memory for control block
		0U                   // size for control block
	},
	{
		"MEMSDMutex",     // human readable mutex name
		osMutexPrioInherit|osMutexRecursive  ,    					// attr_bits
		NULL,                // memory for control block
		0U                   // size for control block
	},
...
#if RTE_USE_OS
	MemoryControlHandle[mem_name].MutexIDMEM = osMutexNew(&MutexIDStdioAttr[mem_name]);
#endif
}
/*************************************************
...
void *RTE_MEM_Alloc(RTE_MEM_Name_e mem_name,uint32_t size)
{
  ...
  void * alloc = NULL;
	...
	RTE_LOG_WARN(MEM_STR,"Couldn't allocate memory size:%d free:%d",size,RTE_MEM_MaxFree(mem_name));
}
	return alloc;
}
/*************************************************
...
void *RTE_MEM_Alloc0(RTE_MEM_Name_e mem_name,uint32_t size)
{
	...
	else RTE_LOG_WARN(MEM_STR,"Couldn't allocate0 memory size:%d",size);
	return alloc;
}
/*************************************************
*** Args:   mem_name 指定的内存区域;
