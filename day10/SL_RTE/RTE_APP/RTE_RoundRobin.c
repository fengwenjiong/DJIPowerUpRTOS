...
#if RTE_USE_SHELL
#include "RTE_Shell.h"
static RTE_Shell_Err_e RTE_Shell_RR_Pause(int argc, char *argv[])
{
    if(argc!=4)
        return SHELL_ARGSERROR;
	RTE_Printf("%10s    Pause Timer:%s %s\r\n",RR_STR,argv[2],argv[3]);
	RTE_RoundRobin_PauseTimer(RTE_RoundRobin_GetGroupID(argv[2]),RTE_RoundRobin_GetTimerID(
                                                        RTE_RoundRobin_GetGroupID(argv[2]),argv[3]));
	return(SHELL_NOERR);
}
static RTE_Shell_Err_e RTE_Shell_RR_Resume(int argc, char *argv[])
{
    if(argc!=4)
        return SHELL_ARGSERROR;
	RTE_Printf("%10s    Resume Timer:%s %s\r\n",RR_STR,argv[2],argv[3]);
	RTE_RoundRobin_ResumeTimer(RTE_RoundRobin_GetGroupID(argv[2]),RTE_RoundRobin_GetTimerID(
                                                        RTE_RoundRobin_GetGroupID(argv[2]),argv[3]));
	return(SHELL_NOERR);
}
#endif // RTE_USE_SHELL
...
#if RTE_USE_SHELL
    RTE_Shell_CreateModule("timer");
    RTE_Shell_AddCommand("timer","pause",RTE_Shell_RR_Pause,"Pause a running timer Example:timer.pause(groupname,timername)");
    RTE_Shell_AddCommand("timer","resume",RTE_Shell_RR_Resume,"Resume a pasuing timer Example:timer.resume(groupname,timername)");
#endif // RTE_USE_SHELL
